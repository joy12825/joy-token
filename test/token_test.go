...
func test(t *testing.T) {
	...
	*/
	gft := gtoken.NewGfToken(
		gtoken.WithCacheKey("gfToken:"),
		gtoken.WithTimeout(60),
		gtoken.WithMaxRefresh(50),
		gtoken.WithMultiLogin(true),
		gtoken.WithExcludePaths(g.SliceStr{"/excludeDemo"}),
		gtoken.WithGRedisConfig(&gredis.Config{
