...
import (
	...
	"github.com/joy12825/gf/container/gtype"
	"github.com/joy12825/gf/debug/gdebug"
...
func (s *Server) routerMapKey(hook HookName, method, path, action, domain string) string {
	return string(hook) + "%" + s.serveHandlerKey(method, path, action, domain)
}
	...
	if handler.Info.Type != nil && handler.Info.Type.NumIn() == 2 {
		...
		if gstr.Contains(method, ",") {
			...
			for _, v := range methods {
				...
				s.doSetHandler(ctx, &clonedHandler, prefix, uri, action, pattern, v, domain)
			}
			return
		}
		// Converts `all` to `ALL`.
		...
		if len(uri) == 0 && len(action) == 0 {
			err = gerror.NewCodef(
				gcode.CodeInvalidParameter,
				`invalid struct for request: defined as "%s", but it should be specify uri or action`, objectReq.Type().Elem().String())
			s.Logger().Fatal(ctx, err)
			return
		}
	}

	s.doSetHandler(ctx, handler, prefix, uri, action, pattern, method, domain)
}

func (s *Server) doSetHandler(
	ctx context.Context, handler *HandlerItem,
	prefix, uri, action, pattern, method, domain string,
) {
