...
func (s *Server) checkAndCreateFuncInfo(f interface{}, pkgPath, structName, methodName string) (info handlerFuncInfo, err error) {
	...
	if !ok {
		...
		if !gstr.HasSuffix(reflectType.In(1).String(), `Req`) && !gstr.HasSuffix(reflectType.In(1).String(), `Action`) {
			err = gerror.NewCodef(
				gcode.CodeInvalidParameter,
				`invalid struct naming for request: defined as "%s", but it should be named with "Req" suffix like "XxxReq" or "xxxAction"`,
