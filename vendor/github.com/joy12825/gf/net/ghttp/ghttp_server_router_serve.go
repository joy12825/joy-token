...
import (
	...
	"github.com/joy12825/gf/container/glist"
	"github.com/joy12825/gf/encoding/gurl"
...
func (s *Server) serveHandlerKey(method, path, action, domain string) string {
	if len(domain) > 0 {
		domain = "@" + domain
	}
	if method == "" {
		return fmt.Sprintf("%s:%s%s", path, action, strings.ToLower(domain))
	}
	return fmt.Sprintf("%s:%s:%s%s", strings.ToUpper(method), path, action, strings.ToLower(domain))
}
...
func (s *Server) getHandlersWithCache(r *Request) (parsedItems []*HandlerItemParsed, serveItem *HandlerItemParsed, hasHook, hasServe bool) {
	...
	var handlerCacheKey = s.serveHandlerKey(method, path, action, host)
	value, err := s.serveCache.GetOrSetFunc(ctx, handlerCacheKey, func(ctx context.Context) (interface{}, error) {
		parsedItems, serveItem, hasHook, hasServe = s.searchHandlers(method, path, action, host)
		if parsedItems != nil {
...
func (s *Server) searchHandlers(method, path, action, domain string) (parsedItems []*HandlerItemParsed, serveItem *HandlerItemParsed, hasHook, hasServe bool) {
	if len(path) == 0 {
	...
	if len(action) > 0 {
		array = []string{action}
	} else if strings.EqualFold("/", path) {
		array = []string{"/"}
	} else {
